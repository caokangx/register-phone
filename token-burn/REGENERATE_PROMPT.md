...
2. 针对每个项目生成 30 条循序渐进的 Claude Code 自然语言任务（不是 shell 命令，是给 `claude -p` 用的 prompt）
...
按任务编号 1–30 分 7 个阶段，每条任务必须标注「任务 X/30」和阶段类型：

| 阶段 | 编号 | 类型 tag | 内容方向 |
|------|------|----------|----------|
| 架构分析 | 1–5 | 架构分析 / 依赖梳理 / 调用链追踪 / 配置审计 / 测试覆盖 | 读目录结构、模块边界、import 关系、配置项、测试用例 |
| 深度源码 | 6–10 | 源码精读 / 并发模型 / 错误处理 / 内存与生命周期 / 扩展点 | 精读核心文件、锁与并发、错误模式、对象生命周期 |
| 性能扫描 | 11–15 | 性能热点扫描 / I/O 路径 / 缓存策略 / 批处理机会 / 基准测试 | 找热点、分析 I/O、审查 cache、设计 benchmark |
| 瓶颈识别 | 16–20 | 瓶颈假设 / 火焰图假想 / 锁竞争 / 内存分配 / 网络往返 | 列瓶颈、推测 profiling 结果、减锁方案 |
| 优化方案 | 21–24 | 优化方案 A–D / 优化优先级 | 写详细方案、伪代码对比、ICE 优先级矩阵 |
| 优化执行 | 25–28 | 代码审查 / 重构草案 / 配置调优 / 降级策略 / 观测增强 | 逐行审查、重构建议、metrics/tracing 方案 |
| 验证报告 | 29–30 | 技术债清单 / 最终报告 | 技术债梳理、全景报告 |

任务生成规则：
- 从 `areas` 和 `topics` 轮换取值，使 30 条内容不重复
...
2. **顺序执行 30 条任务**：读取同目录 `tasks.txt`，用 `claude -p` 逐条执行
...
7. **日志**：`logs/main.log` 总日志 + `logs/task_001.log` … `task_030.log` 每任务日志
