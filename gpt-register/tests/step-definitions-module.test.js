...
test('sidepanel html exposes Plus mode PayPal-only settings and new PayPal SMS / bind-card config rows', () => {
  const html = fs.readFileSync('sidepanel/sidepanel.html', 'utf8');
  ...
  assert.doesNotMatch(html, /<option value="gpc-helper">GPC<\/option>/);
  assert.doesNotMatch(html, /<option value="gopay">GoPay<\/option>/);
  assert.match(html, /id="row-paypal-sms-phone"/);
  assert.match(html, /id="input-paypal-sms-phone"/);
  assert.match(html, /id="row-paypal-sms-api"/);
  assert.match(html, /id="input-paypal-sms-api"/);
  assert.match(html, /id="row-bind-card-number"/);
  assert.match(html, /id="input-bind-card-number"/);
