...
test('step definitions module exposes ordered normal and Plus step metadata', () => {
  ...
  assert.equal(api.getStepById(8, { plusModeEnabled: true, plusPaymentMethod: 'gopay' })?.key, 'gopay-subscription-confirm');
  ...
  assert.equal(api.getLastStepId({ plusModeEnabled: true, plusPaymentMethod: 'gopay' }), 15);
  assert.equal(goPaySteps[5].title, '打开 GoPay 订阅页');
  assert.equal(goPaySteps[6].title, '填写账单并提交订阅');
  assert.equal(goPaySteps[7].title, '跳转并进入 GoPay 支付');
  assert.equal(goPaySteps[12].title, '解绑 GoPay/WhatsApp 接码');
  assert.equal(goPaySteps[13].title, '校验账户余额到账');
