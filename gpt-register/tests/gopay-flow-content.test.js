...
test('GoPay Bayar button accepts additional label text after Bayar', () => {
  const bundle = [
    extractFunction('normalizeText'),
    extractFunction('getActionText'),
    extractFunction('isVisibleElement'),
  ...
  const bayarButton = {
    tagName: 'BUTTON',
    className: 'bg-brand text-white',
    textContent: 'Bayar dengan GoPay',
    innerText: 'Bayar dengan GoPay',
    value: '',
    disabled: false,
    hidden: false,
    parentElement: null,
  ...
  const api = new Function('bayarButton', `
const window = {
  getComputedStyle() { return { display: 'block', visibility: 'visible', opacity: '1' }; },
};
const document = {
  querySelectorAll(selector) {
    return selector.includes('button') || selector.includes('[role="button"]') ? [bayarButton] : [];
  },
};
${bundle}
return { findPayNowButton };
...
test('GoPay terminal timeout page is reported as retry-required state', () => {
  const bundle = [
    ...
    extractFunction('isGoPayOtpInvalidText'),
    extractFunction('isGoPayOtpPageText'),
    extractFunction('isVisibleElement'),
    extractFunction('getVisibleControls'),
    extractFunction('isEnabledControl'),
...
test('GoPay OTP invalid message is reported without terminal failure', () => {
  const bundle = [
    extractFunction('normalizeText'),
    extractFunction('getActionText'),
    extractFunction('getPageBodyText'),
  ...
  const otpInput = {
    ...
    parentElement: null,
    getAttribute(name) {
      if (name === 'placeholder') return 'Kode OTP';
  ...
  const api = new Function('otpInput', `
const location = { href: 'https://gopayapi.com/linking/otp' };
const window = { getComputedStyle() { return { display: 'block', visibility: 'visible', opacity: '1' }; } };
const document = {
  body: { innerText: 'Kode OTP-nya salah. Mohon cek ulang dan coba lagi.', textContent: '' },
  readyState: 'complete',
  querySelectorAll(selector) {
    return String(selector || '').includes('input') ? [otpInput] : [];
  },
};
${bundle}
...
`)(otpInput);
  ...
  const state = api.inspectGoPayState();
  assert.equal(state.otpInvalid, true);
  assert.equal(state.hasOtpInput, true);
  assert.equal(state.hasTerminalError, false);
});
