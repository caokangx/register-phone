...
test('signup phone helper fails fast when page says it cannot send text messages to this phone number', async () => {
  const setStateCalls = [];
  const contentMessages = [];
  const statusActions = [];
  let currentState = {
    ...
    signupPhoneNumber: '66959916439',
    signupPhoneActivation: {
      activationId: 'signup-sms-unusable',
      phoneNumber: '66959916439',
      provider: 'hero-sms',
      serviceCode: 'dr',
      countryId: 52,
  ...
  await assert.rejects(
    ...
    /STEP4_PHONE_NUMBER_UNUSABLE::步骤 4：当前注册手机号已失效/
  );
  ...
  assert.equal(currentState.currentPhoneVerificationCode, '');
  assert.ok(setStateCalls.length > 0);
});

test('signup phone helper stops immediately when resend returns unable-to-send-sms error', async () => {
  const setStateCalls = [];
  const contentMessages = [];
  let currentState = {
    heroSmsApiKey: 'demo-key',
    ...
    signupPhoneActivation: {
      activationId: 'signup-resend-invalid',
      phoneNumber: '447735362840',
      provider: 'hero-sms',
      serviceCode: 'dr',
      countryId: 16,
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    fetchImpl: async (url) => {
      ...
      if (action === 'getStatus' && id === 'signup-resend-invalid') {
        return {
          ok: true,
          text: async () => 'STATUS_WAIT_CODE',
        };
      }
      if (action === 'setStatus' && id === 'signup-resend-invalid') {
        return {
          ok: true,
          text: async () => 'ACCESS_CANCEL',
        };
...
test('signup phone helper stops immediately when resend enters contact-verification 405 route error', async () => {
  const setStateCalls = [];
  const contentMessages = [];
  let currentState = {
    heroSmsApiKey: 'demo-key',
    ...
    phoneCodePollMaxRounds: 1,
    signupPhoneActivation: {
      activationId: 'signup-contact-405',
      phoneNumber: '447735362840',
      provider: 'hero-sms',
      serviceCode: 'dr',
      countryId: 16,
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    fetchImpl: async (url) => {
      ...
      const action = parsedUrl.searchParams.get('action');
      const id = parsedUrl.searchParams.get('id');
      if (action === 'getStatus' && id === 'signup-contact-405') {
        return {
          ok: true,
          text: async () => 'STATUS_WAIT_CODE',
        };
      }
      if (action === 'setStatus' && id === 'signup-contact-405') {
        return {
          ok: true,
          text: async () => 'ACCESS_CANCEL',
        };
    ...
    sendToContentScriptResilient: async (_source, message) => {
      contentMessages.push(message);
      if (message.type === 'RESEND_VERIFICATION_CODE') {
        throw new Error('STEP4_CONTACT_VERIFICATION_ROUTE_405::步骤 4：点击重新发送后进入 /contact-verification 的 405 错误页，当前轮将回到步骤 1 重新开始。URL: https://auth.openai.com/contact-verification');
      }
      throw new Error(`Unexpected content-script message: ${message.type}`);
    },
    setState: async (updates) => {
  ...
  await assert.rejects(
    ...
    /STEP4_CONTACT_VERIFICATION_ROUTE_405::/
  );
...
test('phone verification helper skips page resend for 5sim timeouts and rotates number directly', async () => {
  ...
  assert.equal(messages.filter((type) => type === 'SUBMIT_PHONE_NUMBER').length, 2);
  assert.ok(
    requests.filter((pathname) => pathname === '/v1/user/check/500001').length >= 2,
