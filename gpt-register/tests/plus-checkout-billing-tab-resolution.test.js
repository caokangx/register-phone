    ...
    chrome: {
      ...
      scripting: {
        executeScript: async (details) => {
          events.executeScripts.push(details);
...
test('Plus checkout billing sends fallback contact email for phone-only GoPay checkout', async () => {
  const { events, executor } = createExecutorHarness({
    frames: [
      { frameId: 0, url: 'https://chatgpt.com/checkout/openai_llc/cs_test' },
      { frameId: 7, url: 'https://js.stripe.com/v3/elements-inner-payment.html' },
  ...
  await executor.executePlusCheckoutBilling({
    ...
    ipProxyAppliedExitRegion: 'ID',
  });

  const fillMessage = events.messages.find((entry) => entry.message.type === 'PLUS_CHECKOUT_FILL_BILLING_ADDRESS');
  const contactEmailMessageIndex = events.messages.findIndex((entry) => entry.message.type === 'PLUS_CHECKOUT_FILL_CONTACT_EMAIL');
  const billingMessageIndex = events.messages.findIndex((entry) => entry.message.type === 'PLUS_CHECKOUT_FILL_BILLING_ADDRESS');
  assert.equal(events.messages[contactEmailMessageIndex].frameId, 0);
  assert.equal(contactEmailMessageIndex < billingMessageIndex, true);
  assert.equal(fillMessage.message.payload.contactEmail, '447529768909@example.com');
  const runtimeEmailSyncCall = events.executeScripts.find((entry) => entry.world === 'MAIN');
  assert.ok(runtimeEmailSyncCall);
  assert.deepEqual(runtimeEmailSyncCall.target, { tabId: 42, frameIds: [0] });
  assert.deepEqual(runtimeEmailSyncCall.args, ['447529768909@example.com']);
});
