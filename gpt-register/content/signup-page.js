...
var SIGNUP_PAGE_LISTENER_READY_KEY = '__MULTIPAGE_SIGNUP_PAGE_LISTENER_READY__';

if (!window[SIGNUP_PAGE_LISTENER_READY_KEY]) {
  window[SIGNUP_PAGE_LISTENER_READY_KEY] = true;
...
const LOGIN_SWITCH_TO_PHONE_PATTERN = /继续使用(?:手机|手机号|电话)(?:号码)?登录|使用(?:手机|手机号|电话)(?:号码)?继续|改用(?:手机|手机号|电话)(?:号码)?登录|手机号登录|continue\s+(?:with|using)\s+(?:a\s+)?phone(?:\s+number)?|use\s+(?:a\s+)?phone(?:\s+number)?(?:\s+instead)?|sign\s*in\s+with\s+(?:a\s+)?phone/i;
...
async function resendVerificationCode(step, timeout = 45000) {
  ...
  while (Date.now() - start < timeout) {
    ...
    if (is405MethodNotAllowedPage()) {
      if (isStep4ContactVerificationRoute405ErrorPage(step)) {
        log('步骤 4：检测到 /contact-verification 的 405 错误页，当前轮将回到步骤 1 重新开始。', 'warn');
        throw createStep4ContactVerificationRoute405Error();
      }
    ...
    if (action && isActionEnabled(action)) {
      ...
      if (is405MethodNotAllowedPage()) {
        if (isStep4ContactVerificationRoute405ErrorPage(step)) {
          log('步骤 4：点击重新发送后进入 /contact-verification 的 405 错误页，当前轮将回到步骤 1 重新开始。', 'warn');
          throw createStep4ContactVerificationRoute405Error({ afterResend: true });
        }
      ...
      if (Number(step) === 4) {
        const unusableText = getStep4PhoneNumberUnusableText();
        if (unusableText) {
          log(`步骤 4：点击重新发送后检测到当前手机号不可用：${unusableText}`, 'warn');
          throw createStep4PhoneNumberUnusableError(unusableText);
        }
      }

      const errorText = getVerificationErrorText();
      if (errorText) {
        log(`步骤 ${step}：点击重新发送后页面提示错误：${errorText}`, 'warn');
        return {
          invalidCode: true,
          errorText,
...
function isStep4ContactVerificationRoute405ErrorPage(step) {
  if (Number(step) !== 4) {
    return false;
  }
  const pageText = document.body?.textContent || '';
  if (!AUTH_ROUTE_ERROR_PATTERN.test(pageText)) {
    return false;
  }
  const routeText = `${location.pathname || ''} ${location.href || ''} ${pageText}`;
  return /\/contact-verification(?:[/?#]|$)|\bCONTACT_VERIFICATION\b/i.test(routeText);
}

function createStep4ContactVerificationRoute405Error(options = {}) {
  const leadIn = options?.afterResend
    ? '步骤 4：点击重新发送后进入 /contact-verification 的 405 错误页，当前轮将回到步骤 1 重新开始。'
    : '步骤 4：检测到 /contact-verification 的 405 错误页，当前轮将回到步骤 1 重新开始。';
  return new Error(`STEP4_CONTACT_VERIFICATION_ROUTE_405::${leadIn}URL: ${location.href}`);
}
...
const STEP4_PHONE_NUMBER_UNUSABLE_ERROR_PREFIX = 'STEP4_PHONE_NUMBER_UNUSABLE::';
const STEP4_PHONE_NUMBER_UNUSABLE_PATTERN = /unable\s+to\s+send\s+(?:a\s+)?(?:text\s+messages?|sms|verification\s+code)s?\s+to\s+this\s+phone\s+number|can(?:not|'t)\s+send\s+(?:a\s+)?(?:text\s+messages?|sms|verification\s+code)s?\s+to\s+this\s+phone\s+number|couldn'?t\s+send\s+(?:a\s+)?(?:text\s+messages?|sms|verification\s+code)s?\s+to\s+this\s+phone\s+number|无法向此电话(?:号|号码)发送(?:文本消息|短信|验证码)|无法向此手机号发送(?:文本消息|短信|验证码)|无法向此电话号码发送(?:文本消息|短信|验证码)|不能向此电话(?:号|号码)发送(?:文本消息|短信|验证码)|不能向此手机号发送(?:文本消息|短信|验证码)|不能向此电话号码发送(?:文本消息|短信|验证码)/i;
...
function getStep4PhoneNumberUnusableText() {
  const candidates = [
  ...
  for (const candidate of candidates) {
    ...
    if (!text || !STEP4_PHONE_NUMBER_UNUSABLE_PATTERN.test(text)) {
      continue;
    }
    const match = text.match(STEP4_PHONE_NUMBER_UNUSABLE_PATTERN);
    return String(match?.[0] || text).trim().slice(0, 300);
  }

  return '';
}

function createStep4PhoneNumberUnusableError(errorText = '') {
  const detail = String(errorText || '').replace(/\s+/g, ' ').trim();
  return new Error(
    `${STEP4_PHONE_NUMBER_UNUSABLE_ERROR_PREFIX}步骤 4：当前注册手机号已失效，无法继续接收短信${detail ? `：${detail}` : '。'}`
  );
}
...
function findSessionSelectionButton() {
  ...
  return candidates.find((button) => isVisibleElement(button) && isActionEnabled(button)) || null;
}

function isSessionSelectionPageReady() {
  const path = `${location.pathname || ''} ${location.href || ''}`;
  if (/\/session-selection(?:[/?#]|$)|\/sessions?\/select(?:[/?#]|$)/i.test(path)) {
    return true;
  }
  if (findSessionSelectionButton()) {
    return true;
  }
  return false;
}
...
function findLoginPhoneEntryTrigger() {
  const direct = document.querySelector?.('button[data-dd-action-name="Continue with phone"]') || null;
...
function clickLoginChoiceControl(target) {
  throwIfStopped();
  if (!target) {
    throw new Error('登录方式切换按钮不存在。');
  }
...
function inspectLoginAuthState() {
  ...
  const sessionSelectionButton = findSessionSelectionButton();
  const sessionSelectionPage = !verificationTarget
    && !phoneVerificationPage
    && !addPhonePage
    && !addEmailPage
    && Boolean(sessionSelectionButton);
  const baseState = {
    state: 'unknown',
    url: location.href,
  ...
  if (sessionSelectionPage) {
    return {
      ...baseState,
      state: 'session_selection_page',
    };
  }
...
function serializeLoginAuthState(snapshot) {
  return {
    ...
    sessionSelectionPage: Boolean(snapshot?.sessionSelectionPage),
  };
}
...
function buildPhoneLoginUrl(rawUrl = location.href) {
  try {
    const parsed = new URL(rawUrl || location.href);
    parsed.pathname = '/log-in';
    parsed.searchParams.set('usernameKind', 'phone_number');
    return parsed.toString();
  } catch {
    const base = String(rawUrl || location.href || 'https://auth.openai.com/log-in').split('#')[0];
    const separator = base.includes('?') ? '&' : '?';
    return `${base}${separator}usernameKind=phone_number`;
  }
}

async function forcePhoneLoginEntryByUrl(timeout = 20000, options = {}) {
  const visibleStep = Math.floor(Number(options?.visibleStep) || 0) || 7;
  const targetUrl = buildPhoneLoginUrl();
  log(`步骤 ${visibleStep}：点击手机号登录入口后页面仍停留在邮箱页，改用 URL 参数切换手机号登录。`, 'warn', {
    step: visibleStep,
    stepKey: 'oauth-login',
  });
  ...
  return normalizeStep6Snapshot(await waitForPhoneLoginEntrySwitchTransition(timeout));
}

async function step6SelectExistingSession(payload, snapshot) {
  ...
  const trigger = currentSnapshot.sessionSelectionButton || findSessionSelectionButton();
  if (!trigger) {
    return createStep6RecoverableResult('missing_session_selection_button', currentSnapshot, {
      message: '认证页提示选择已登录账户，但未找到可点击的会话按钮。',
    });
  }

  log(`检测到选择已登录账户页，正在点击 "${getActionText(trigger).slice(0, 80)}" 继续...`, 'info', { step: visibleStep, stepKey: 'oauth-login' });
  await humanPause(350, 900);
  ...
  while (Date.now() - start < 15000) {
    throwIfStopped();
    await sleep(250);
    const inspected = normalizeStep6Snapshot(inspectLoginAuthState());
    if (inspected.state && inspected.state !== 'unknown' && inspected.state !== 'session_selection_page') {
      nextSnapshot = inspected;
      break;
    }
    nextSnapshot = inspected;
  }
  ...
  if (nextSnapshot.state === 'add_phone_page') {
    log('选择已登录账户后页面进入添加手机号页，由 add-phone 流程接管。', 'info', { step: visibleStep, stepKey: 'oauth-login' });
    throwForStep6FatalState(nextSnapshot, visibleStep);
  }
  if (nextSnapshot.state === 'add_email_page') {
    return createStep6AddEmailSuccessResult(nextSnapshot, {
      via: 'session_selection_add_email_page',
    });
  }
  if (nextSnapshot.state === 'oauth_consent_page') {
    return createStep6OAuthConsentSuccessResult(nextSnapshot, {
      via: 'session_selection_oauth_consent_page',
    });
  }
  ...
  if (nextSnapshot.state === 'session_selection_page') {
    return createStep6RecoverableResult('session_selection_stalled', nextSnapshot, {
      message: '点击选择已登录账户后页面未变化，可能需要手动确认。',
    });
  }

  return createStep6RecoverableResult('session_selection_unknown', nextSnapshot, {
    message: `选择已登录账户后页面进入了未识别状态：${getLoginAuthStateLabel(nextSnapshot)}。`,
  });
}
...
async function switchFromEmailPageToPhoneLogin(payload, snapshot) {
  ...
  if (!phoneEntryTrigger || !isActionEnabled(phoneEntryTrigger)) {
    ...
    if (moreOptionsTrigger && isActionEnabled(moreOptionsTrigger)) {
      ...
      await humanPause(350, 900);
      clickLoginChoiceControl(moreOptionsTrigger);
      await sleep(800);
  ...
  await humanPause(350, 900);
  clickLoginChoiceControl(phoneEntryTrigger);
  let nextSnapshot = normalizeStep6Snapshot(await waitForPhoneLoginEntrySwitchTransition(20000));
  if (nextSnapshot.state === 'email_page' || nextSnapshot.state === 'entry_page') {
    nextSnapshot = await forcePhoneLoginEntryByUrl(20000, { visibleStep });
  }
  if (nextSnapshot.state === 'phone_entry_page') {
    return step6LoginFromPhonePage(payload, nextSnapshot);
  }
...
async function step6_login(payload) {
  ...
  let snapshot;
  if (payload?.forcePhoneLoginOnAuthPage && phoneNumber && !isLoginPhoneUsernameKind()) {
    log(`步骤 ${visibleStep}：收到第 10 步强制手机号登录标志，正在将登录页切换为手机号模式...`, 'info', {
      step: visibleStep,
      stepKey: 'oauth-login',
    });
    snapshot = await forcePhoneLoginEntryByUrl(20000, { visibleStep });
  } else {
    snapshot = normalizeStep6Snapshot(await waitForKnownLoginAuthState(15000));
  }

  if (snapshot.state === 'session_selection_page') {
    return step6SelectExistingSession(payload, snapshot);
  }
