  ...
  function createSub2ApiApi(deps = {}) {
    ...
    async function generateOpenAiAuthUrl(state = {}, options = {}) {
      ...
      const groupNames = normalizeSub2ApiGroupNames(
        Array.isArray(state.sub2apiSelectedGroupNames) && state.sub2apiSelectedGroupNames.length
          ? state.sub2apiSelectedGroupNames
          : (state.sub2apiGroupName || DEFAULT_SUB2API_GROUP_NAME)
      );
    ...
    async function submitOpenAiCallback(state = {}, options = {}) {
      ...
      const fallbackGroupNames = Array.isArray(state.sub2apiSelectedGroupNames) && state.sub2apiSelectedGroupNames.length
        ? state.sub2apiSelectedGroupNames
        : (state.sub2apiGroupName || DEFAULT_SUB2API_GROUP_NAME);
      const groups = groupIdsFromState.length
        ? groupIdsFromState.map((id) => ({ id }))
        : (state.sub2apiGroupId && !(Array.isArray(state.sub2apiSelectedGroupNames) && state.sub2apiSelectedGroupNames.length > 1)
          ? [{ id: state.sub2apiGroupId, name: state.sub2apiGroupName || DEFAULT_SUB2API_GROUP_NAME }]
          : await getGroupsByNames(origin, token, fallbackGroupNames, options));
