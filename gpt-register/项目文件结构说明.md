...
- `background.js`：扩展后台 Service Worker 入口壳，负责模块装配、初始化、全局常量、少量保留的领域函数与运行入口；当前新增 `oauthFlowTimeoutEnabled` 持久化配置，并统一承接 OAuth 授权后链总预算开关与剩余预算计算；`DEFAULT_SIGNUP_METHOD = 'phone'`、`phoneVerificationEnabled` 默认开，`canUsePhoneSignup` 不再被 Plus 模式拦截（仅贡献模式与缺接码会拦截），允许 Plus + 手机号注册组合。
...
- `background/message-router.js`：后台消息路由层，负责处理 `chrome.runtime.onMessage` 进入的所有业务消息；`LOG / STEP_COMPLETE / STEP_ERROR` 会把消息里的真实 `step` 继续传给结构化日志，跳过登录验证码这类派生日志也按当前步骤 key 写入；当前额外接入 PayPal 账号池的新增与切换、2925 账号池的新增、导入、切换、登录、禁用与删除消息，以及 IP 代理的同步、切换、Change 与出口检测消息；手动改邮箱、手动取邮箱等入口也统一经这里落到 `setEmailState`，同步维护注册邮箱运行态；当侧栏关闭 `oauthFlowTimeoutEnabled` 时，会立即清空已存在的 OAuth 总预算 deadline；额外提供 `BG_FETCH_MEIGUODIZHI_ADDRESS` 跨域桥，供 hosted Stripe 和 PayPal 内容脚本从 meiguodizhi.com 取美国地址。
...
- `background/sub2api-api.js`：SUB2API 管理接口直连模块，负责后台登录 SUB2API、解析 openai 分组、按配置选择代理、生成 OpenAI OAuth 地址、交换 localhost callback 授权码并创建 SUB2API 账号；主链路不再依赖 SUB2API 后台页面 DOM 自动化；分组解析优先读取 `state.sub2apiSelectedGroupNames` 数组，未配置时回退到旧 `sub2apiGroupName`，账号创建时按所有命中分组一次性 `group_ids` 多绑。
...
- `background/steps/create-plus-checkout.js`：Plus 模式第 6 步实现，负责在已登录 ChatGPT 页面通过 `content/plus-checkout.js` 调 `/backend-api/payments/checkout` 创建 hosted 长链接（PayPal-only，固定 US/USD/promo-plus-1-month-free/cancel_url=#pricing），再把当前标签更新到长链接所在的 Stripe hosted 页面；旧 GoPay / GPC 分支已删除。
...
- `background/steps/fill-plus-checkout.js`：Plus 模式第 7 步实现，作为 thin wrapper：定位 `pay.openai.com` / `checkout.stripe.com` 标签页、注入 `content/plus-checkout.js`、发 `RUN_HOSTED_CHECKOUT_FLOW` 让内容脚本点 PayPal accordion + 填美国账单地址 + 勾选服务条款 + 点提交，然后等待标签页跳转到 `paypal.com`。旧的 Stripe 自定义 UI iframe / GoPay / GPC 分支已删除。
...
- `background/steps/paypal-approve.js`：Plus 模式第 8 步实现，作为 thin wrapper：定位 PayPal 标签页、按当前 URL 分发 `PAYPAL_RUN_GUEST_LOGIN` (`/pay`) 或 `PAYPAL_RUN_GUEST_CHECKOUT` (`/checkoutweb/`) 给内容脚本；`/checkoutweb` 提交需要侧边栏的「绑卡」卡号和「PayPal 手机」配置项；提交后停在 PayPal 短信验证页（自动接码暂未实现）。旧的 PayPal 账号密码登录 + passkey + 授权点击逻辑已删除。
...
- `content/paypal-flow.js`：PayPal 页面脚本，提供 `PAYPAL_GET_STATE` / `PAYPAL_RUN_GUEST_LOGIN`（在 `/pay` 填随机 gmail 邮箱并下一步）/ `PAYPAL_RUN_GUEST_CHECKOUT`（在 `/checkoutweb` 强制 US、用 payload 里 `cardNumber`、`phone`、写死 expiry `03 / 30` 和 cvv `996`、随机 email/password、固定姓名 James Smith、通过 background `BG_FETCH_MEIGUODIZHI_ADDRESS` 拿美国地址并提交）。
...
- `content/plus-checkout.js`：ChatGPT Plus checkout 页面脚本，双角色：① 在 chatgpt.com 上读取 `/api/auth/session` accessToken 并调 `/backend-api/payments/checkout` 创建 hosted 长链接（`CREATE_PLUS_CHECKOUT`，固定 PayPal/US/USD/promo-plus-1-month-free/cancel_url=#pricing），返回 `data.url || stripe_hosted_url || checkout_url`；② 在 `pay.openai.com` / `checkout.stripe.com` 的 hosted 页面上响应 `RUN_HOSTED_CHECKOUT_FLOW`：点击 PayPal accordion、通过 background `BG_FETCH_MEIGUODIZHI_ADDRESS` 拿美国地址、填 `#billingAddressLine1` / `#billingLocality` / `#billingPostalCode` 与州下拉框、勾 `#termsOfServiceConsentCheckbox`、点 hosted submit 按钮。旧的 Stripe 自定义 UI iframe 操作、Google 地址推荐拆分等代码已移除。
...
- `content/sub2api-panel.js`：SUB2API 后台内容脚本，保留后台页面内生成 OAuth 地址和提交 localhost 回调的兼容能力；当前主流程已迁移到 `background/sub2api-api.js` 的管理接口直连路径；分组解析同样优先读取 `payload.sub2apiSelectedGroupNames` 数组（或 backgroundState 的同名字段），未配置时回退到 `sub2apiGroupName`，支持创建账号时多分组绑定。
...
- `sidepanel/sidepanel.css`：侧边栏样式文件；当前额外提供 Hotmail / 2925 共用的号池表单容器、操作按钮行、统一的收起态列表高度样式，以及 IP 代理配置区、状态卡和操作按钮样式；为 SUB2API 分组多选 picker 增加 `editable-list-option-multi` 复选框行样式。
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  钥配置行；SUB2API 分组选择器改为多选模式（仍保留新增/删除分组入口），底层会持久化 `sub2apiSelectedGroupNames` 数组；设置卡片新增 `IP代理` 开关与代理配置折叠区，支持 711Proxy 账号密码模式、代理状态卡与出口检测按钮；接码卡内把注册方式、接码配置、接码订单运行态与“注册手机号”身份运行态分层展示，“注册手机号”位于订单运行状态下方，不嵌入当前分配/验证码网格；当前固定走 Plus 模式 + PayPal 支付链路，旧的 `Plus 模式` 开关、`Plus 支付` 下拉与 `PayPal 账号` 下拉框 / GoPay / GPC 行常驻隐藏，并新增 `PayPal 手机` / `PayPal API` / `绑卡（0 刀卡卡号）` 三个常驻配置行（前端配置项，后端实现暂未接入）；Hotmail / 2925 两个账号池当前都使用统一的头部“添加账号/取消添加”按钮和共享表单容器；设置卡片还新增“授权总超时”开关，用于控制 Step 7 后链 5 分钟总预算。
...
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、共享验证码自动重发次数配置与广播接收，并装
  ...
  在普通失败时继续复用当前邮箱，只有成功或出现手机号验证时才切换下一个邮箱；当前流程固定走 Plus 模式 + PayPal 支付链路，`updatePlusModeUI` 强制保留对应行为隐藏并锁定 `inputPlusModeEnabled=true`、`selectPlusPaymentMethod=paypal`，旧的 PayPal 账号下拉与 GoPay / GPC 行常驻隐藏，同时持久化新的 `paypalSmsPhone` / `paypalSmsApiUrl` / `bindCardNumber` 配置项；SUB2API 分组使用内嵌的多选 picker，落地 `sub2apiSelectedGroupNames`，第一项同步回兼容字段 `sub2apiGroupName`；Step 8 的自定义邮箱确认弹
...
- `tests/paypal-flow-content.test.js`：测试 PayPal 内容脚本对 `/pay` 随机邮箱+下一步、`/checkoutweb` 强制 US + 用 payload 卡号/手机号 + 写死 expiry/cvv + 随机邮箱密码 + 后台桥地址 + 提交，以及缺少卡号/手机号时报错的分支。
- `tests/plus-checkout-address-input.test.js`：测试 Plus checkout 内容脚本的可重复注入、hosted `CREATE_PLUS_CHECKOUT` 请求按脚本默认参数发出并接受 `url / stripe_hosted_url / checkout_url` 三种长链接字段，以及 `RUN_HOSTED_CHECKOUT_FLOW` 在 hosted 页面点 PayPal accordion、填后台桥地址、勾服务条款、点提交。
